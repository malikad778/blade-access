...
class TextareaMissingLabelRule extends AbstractRule
{
    public function getId(): string
    {
        return 'textarea-missing-label';
    }

    public function getDescription(): string
    {
        return 'Textarea elements must have accessible labels.';
    }
    ...
    {
        return 'Add a <label> with a "for" attribute matching the textarea\'s "id", or use aria-label/aria-labelledby.';
    }
    ...
    public function check(array $ast, RuleContext $context): array
    {
        ...
        foreach ($ast as $node) {
            if (($node['nodeType'] ?? '') === 'Element' && strtolower($node['tagName'] ?? '') === 'textarea') {
                ...
                if (!$hasAriaLabel && !$hasImplicitLabel && !$hasExplicitLabel) {
                    $violations[] = $this->makeViolation(
                        ...
                        $node['line'] ?? 1,
                        $node['column'] ?? 1,
                        '<textarea> element is missing an accessible label.',
